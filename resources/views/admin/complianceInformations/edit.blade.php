        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.complianceInformation.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.compliance-informations.update", [$complianceInformation->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('aware_of_international_law') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="aware_of_international_law" id="aware_of_international_law">
                                ...
                                @foreach(App\Models\ComplianceInformation::AWARE_OF_INTERNATIONAL_LAW_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('aware_of_international_law', $complianceInformation->aware_of_international_law) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('fisheries_related_offences') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="fisheries_related_offences" id="fisheries_related_offences">
                                ...
                                @foreach(App\Models\ComplianceInformation::FISHERIES_RELATED_OFFENCES_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('fisheries_related_offences', $complianceInformation->fisheries_related_offences) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
