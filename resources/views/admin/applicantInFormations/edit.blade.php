        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.applicantInFormation.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.applicant-in-formations.update", [$applicantInFormation->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('full_name_of_applicant') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="full_name_of_applicant" id="full_name_of_applicant" value="{{ old('full_name_of_applicant', $applicantInFormation->full_name_of_applicant) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('applicant_citizenship') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="applicant_citizenship" id="applicant_citizenship" required>
                                ...
                                @foreach(App\Models\ApplicantInFormation::APPLICANT_CITIZENSHIP_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('applicant_citizenship', $applicantInFormation->applicant_citizenship) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('email_address') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="email_address" id="email_address" value="{{ old('email_address', $applicantInFormation->email_address) }}">
                        ...
                        <div class="form-group {{ $errors->has('phone_or_mobile_nu') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone_or_mobile_nu" id="phone_or_mobile_nu" value="{{ old('phone_or_mobile_nu', $applicantInFormation->phone_or_mobile_nu) }}">
                        ...
                        <div class="form-group {{ $errors->has('work_address') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="work_address" id="work_address" value="{{ old('work_address', $applicantInFormation->work_address) }}">
